...
function EditProfileContent() {
  ...
  const handleButtonClick = () => {
    if (!isEditing) {
      // Navigate to profile when confirmed
      router.push("/profile");
    } else {
    ...
    <ProtectedPage>
      <div className="relative flex flex-col min-h-screen bg-primary">
        ...
        <div className="profile-container flex flex-col items-center mt-4 relative z-10">
          ...
          <div className="w-full flex justify-between px-4 space-x-4">
            <button
              onClick={handleButtonClick}
              className={
                "justify-end text-sm text-white py-2 px-4 rounded bg-[#5a88ff]"
              }
            >
              Upload Picture
            </button>
            <button
              onClick={handleButtonClick}
              className={
                "justify-end text-sm text-white py-2 px-4 rounded bg-green-500"
              }
            >
              Confirm
            </button>
            ...
            {activeTab === "My Spots" &&
              mySpots.map((spot) => (
                ...
                >
                  ...
                  {/* Red X overlay */}
                  <div
                    className="absolute top-0 left-0 w-full h-full bg-red-500 bg-opacity-50 flex items-center justify-center cursor-pointer"
                    onClick={async () => {
                      const confirmDelete = confirm(
                        `Are you sure you want to delete the spot: ${spot.spotName}?`
                      );
                      if (confirmDelete) {
                        try {
                          ...
                          if (response.status === 204) {
                            ...
                            setMySpots((prevSpots) =>
                              prevSpots.filter((s) => s.id !== spot.id)
                            );
                          } else if (response.status === 403) {
                            alert(
                              "You are not authorized to delete this spot."
                            );
                          } else {
                            const data = await response.json();
                            alert(`Failed to delete spot: ${data.error}`);
                          }
                        } catch (error) {
